...
/**
 * True when a site is still a placeholder/unconfigured — i.e. its baseURL is
 * missing, unparseable, or points at a reserved `example.(com|org|net)` domain.
 * The bundled example specs use this to skip themselves so CI is green out of
 * the box; once a site points at a real URL, its specs run.
 * @param {{ baseURL?: string }} config
 */
function isExampleSite(config) {
  try {
    const host = new URL(config.baseURL).hostname;
    return /(^|\.)example\.(com|org|net)$/i.test(host);
  } catch {
    return true; // no/invalid baseURL => treat as not configured
  }
}
